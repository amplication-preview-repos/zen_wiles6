...
const service = {
  ...
  boardMembers: () => FIND_MANY_RESULT,
  boardMember: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("BoardMember", () => {
  ...
  test("GET /boardMembers/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/boardMembers"}/${nonExistingId}`)
  ...
  test("GET /boardMembers/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/boardMembers"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /boardMembers existing resource", async () => {
