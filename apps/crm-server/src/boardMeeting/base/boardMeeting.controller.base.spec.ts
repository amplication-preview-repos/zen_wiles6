...
const service = {
  ...
  boardMeetings: () => FIND_MANY_RESULT,
  boardMeeting: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("BoardMeeting", () => {
  ...
  test("GET /boardMeetings/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/boardMeetings"}/${nonExistingId}`)
  ...
  test("GET /boardMeetings/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/boardMeetings"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /boardMeetings existing resource", async () => {
